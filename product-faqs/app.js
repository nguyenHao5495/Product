...
function omgfaqs_getJsonFile() {
  ...
  $.ajax({
    dataType: "json",
    url:
      rootlinkProductFaqs + "/cache/" + omgfaqs_shopName + "/data.json?v=" + v,
  }).done(function (data) {
    omgfaqs_storeSettings = data.app.settings;
    if (omgfaqs_productId != null) {
      jQuery.get(
        rootlinkProductFaqs + "/productfaqs.php",
        {
...
function omgfaqs_getProductFaqsContent(lists, count) {
  var emptyQuestion =
      omgfaqs_countFaqs == 0
        ? `<div class="ot-faqs-notice-no-question">No question yet</div>`
        : "",
    loadmore =
      lists.length == omgfaqs_storeSettings.per_page
        ? `
            <div>
...
function omgfaqs_getProductAnswer(element) {
  ...
  var like = parseInt(element.vote) > 1 ? "Votes" : "Vote",
    voteDown =
      parseInt(element.vote) > 0
        ? `<a id="question-vote-down-${element.id}" class="question-vote-button question-vote-down" onclick="voteDownQuestion(event,${element.id})"></a>`
        : "",
    lock =
      element.id == 1
        ? `<span class="locked-question"><img title="Locked question" src="${rootlinkProductFaqs}/assets/images/locked.png"></span>`
        : "",
    html = "",
    answerHtml = "",
    reply = "";
  if (listAnswer.length > 0) {
    ...
    if (listAnswer.length > 1)
      answerHtml +=
        `
                        <div class="loadmore-answer"><a onclick="seeMoreAnswers(${element.id})"><img src="${rootlinkProductFaqs}/assets/images/arrow-double-down.png" style="width:8px;"> ${omgfaqs_storeSettings.see_more_answer} (` +
        (listAnswer.length - 1) +
        `)</a></div>
  ...
  html +=
    `<div class="question-item" id="question-item-${element.id}">
...
function seeMoreAnswers(id) {
  jQuery(".ot-product-faqs #question-item-" + id + " .more-answer").show(
    "slow"
  );
  jQuery(".ot-product-faqs #question-item-" + id + " .collapse-answer").show(
    "slow"
  );
  jQuery(".ot-product-faqs #question-item-" + id + " .loadmore-answer").hide(
    "slow"
  );
}
function collapseAllAnswers(id) {
  jQuery(".ot-product-faqs #question-item-" + id + " .loadmore-answer").show(
    "slow"
  );
  jQuery(".ot-product-faqs #question-item-" + id + " .more-answer").hide(
    "slow"
  );
  jQuery(".ot-product-faqs #question-item-" + id + " .collapse-answer").hide(
    "slow"
  );
}
...
function omgfaqs_submitQuestion(e) {
  e.preventDefault();
  $(".ot-product-faqs .question-form #question-form-submit")
    .prop("disabled", true)
    .css("cursor", "default");
  if (omgfaqs_shopName == "product-questions-and-answers-demo.myshopify.com") {
    $(".ot-product-faqs .question-form #question-form-submit").prop(
      "disabled",
      false
    );
    ...
    var form_data = new FormData(),
      question = {
        ...
        faqQuestion: $(
          ".ot-product-faqs .question-form #faqs_question input"
        ).val(),
      };
...
function submitReply(e, id) {
  ...
  var form_data = new FormData(),
    answer = {
      faqName: $(
        ".ot-product-faqs #question-item-" +
          id +
          " .reply-form .reply_name input"
      ).val(),
      faqEmail: $(
        ".ot-product-faqs #question-item-" +
          id +
          " .reply-form .reply_email input"
      ).val(),
      faqQuestion: $(
        ".ot-product-faqs #question-item-" +
          id +
          " .reply-form .reply_answer input"
      ).val(),
    };

  jQuery(
    ".ot-product-faqs #question-item-" + id + " .reply-form #reply-form-submit"
  )
    .prop("disabled", true)
    .css("cursor", "default");
  form_data.append("shop", omgfaqs_shopName);
  ...
  axios
    .post(rootlinkProductFaqs + "/productfaqs.php", form_data)
    .then(function (res) {
      $(".ot-product-faqs #question-item-" + id + " .close-reply-form").hide(
        "slow"
      );
      $(".ot-product-faqs #question-item-" + id + " .reply-form").hide("slow");
      $(".ot-product-faqs #question-item-" + id + " .open-reply-form").hide(
        "slow"
      );
      if (res.data == true) {
        $("#question-item-" + id + " .ot-faqs-notice-addreply-success").fadeIn(
          300
        );
      } else {
        $("#question-item-" + id + " .ot-faqs-notice-addreply-error").fadeIn(
          300
        );
      }
    })
...
function getMoreQuestion(e) {
  e.preventDefault();
  var offset = Number(
      $(".ot-product-faqs .question-answer-box .question-item").length
    ),
    form_data = new FormData();
  $(".ot-product-faqs .question-loadmore")
    .prop("disabled", true)
    .css("cursor", "default");
  form_data.append("shop", omgfaqs_shopName);
  ...
  axios
    ...
    .then(function (res) {
      ...
      $(".ot-product-faqs .question-answer-box .question-answer-list").append(
        html
      );
      if (
        Number(
...
function voteUpQuestion(e, id) {
  ...
  if (meta.page.customerId) {
    ...
    jQuery(location).attr(
      "href",
      "https://" + omgfaqs_shopName + "/account/login"
    );
  }
}
